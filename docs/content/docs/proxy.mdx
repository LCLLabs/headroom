...
| `--session-delta-log` | `false` | Append message-level post-processing deltas per session (Claude Code + Codex). Also `HEADROOM_SESSION_DELTA_LOG=1` |
...
headroom proxy --session-delta-log
